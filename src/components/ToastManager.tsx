...
export const ToastManager = () => {
  ...
  return (
    <div className="absolute z-50 top-10 left-0 w-full flex flex-col items-center gap-y-4 pointer-events-none bg-red-300">
      {toasts.map(({ id, type, message, state }) => {
        const Icon = ICON_MAP[type];
        return (
          <div
            key={id}
            className={`flex items-start gap-x-2 min-w-60 px-6 py-3 bg-neutral-800/90 rounded-lg text-white text-base shadow-[0_0_10px_rgba(0,0,0,0.4)] 
              ${
